# 021 — Alerting on Manual (Non-Terraform) Changes

**Goal:** turn the manual detection from
[020_configuration_drift](../020_configuration_drift) into a passive
...
same way [019_state_bucket_least_privilege](../019_state_bucket_least_privilege)
...
different *kind* of signal than 019's IAM boundary. Keep that
...
6. Drift the bucket by hand, exactly like
   [020_configuration_drift](../020_configuration_drift) step 3:
   ```bash
...
7. Now fix the drift with `terraform apply` (Option A from 020) and
...
[019_state_bucket_least_privilege](../019_state_bucket_least_privilege)'s
